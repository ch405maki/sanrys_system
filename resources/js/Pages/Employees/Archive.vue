<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center p-[4px]">
                <div>
                    <h2 class="font-semibold text-xl text-rose-500 leading-tight">Employees Archive / <span class="text-xs font-normal text-rose-600">Manage resigned employees</span></h2>
                </div>
                <div>
                    <!-- Branch Filter -->
                    <select v-model="selectedFilterBranch" class="border border-gray-300 rounded-md p-2 w-32 mr-2">
                        <option value="">All Branches</option>
                        <option v-for="branch in uniqueBranches" :key="branch" :value="branch">
                            {{ branch }}
                        </option>
                    </select>
                    <a href="" class="bg-green-100 hover:bg-green-200 py-[10px] px-4 rounded-md border border-green-500 text-gray-700 mr-2"> <i class="fa-solid fa-download mr-2"></i> Download</a>
                </div>
            </div>
        </template>
                                ...
                                <table class="table-auto min-w-full rounded-xl">
                                    <thead>
                                        <tr class="bg-rose-100">
                                            ...
                                            <th scope="col" class="p-5 text-center whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize"> Status </th>
                                    ...
                                    <tbody class="divide-y divide-gray-300">
                                        <tr v-for="employee in filteredEmployees" :key="employee.id" class="bg-white transition-all duration-500 hover:bg-rose-50">
                                            <td class="px-5 py-3">
                                            ...
                                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                <div class="py-1.5 px-2.5 rounded-full flex justify-center items-center gap-1">
                                                    <span class="font-medium text-xs text-rose-600 uppercase"><i class="fa-solid fa-chart-simple mr-2"></i>{{ employee?.status }}</span>
                                                </div>
                                            </td>
                                            <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                <div class="flex justify-center items-center gap-2">
                                                    <a :href="route('employee.show', employee.id)"class="p-2 rounded-full text-green-700 hover:text-white bg-white group transition-all duration-500 hover:bg-green-500 flex item-center">
                                                        <svg class="w-6 h-6 " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                                            <path stroke="currentColor" stroke-width="2" d="M21 12c0 1.2-4.03 6-9 6s-9-4.8-9-6c0-1.2 4.03-6 9-6s9 4.8 9 6Z"/>
                                                            <path stroke="currentColor" stroke-width="2" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                                        </svg>
                                                    </a>
                                                    <a  :href="route('employee.edit', employee.id)" class="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
                                                    ...
                                                    <button @click="confirmDeleteEmployee(employee.id)" class="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-red-600 flex item-center">
                                                    ...
                                                    <button @click="openUpdateModal(employee)" class="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
                                                        <svg class="fill-indigo-500 group-hover:fill-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="isUpdateModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div class="bg-white rounded-lg p-6 w-96">
                ...
                <div class="flex justify-end gap-2">
                    <button @click="closeUpdateModal" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-md">Cancel</button>
                    <button @click="confirmUpdateStatus" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md">Update</button>
                </div>
            </div>
        </div>

        <!-- Delete Confirmation Modal -->
        <div v-if="confirmDelete" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div class="bg-white p-6 rounded-lg w-96">
                <h2 class="text-lg font-semibold mb-4">Confirm Deletion</h2>
                <p class="text-sm text-gray-600 mb-6">Are you sure you want to delete this employee? This action cannot be undone.</p>
                <div class="flex justify-end gap-2">
                    <button @click="confirmDelete = false" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-md">Cancel</button>
                    <button @click="deleteEmployee(selectedId)" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md">Delete</button>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
...
const selectedFilterBranch = ref('');

// Extract unique statuses dynamically
const uniqueStatuses = computed(() => {
    const statuses = props.employees.map(employee => employee.status).filter(Boolean);
    return [...new Set(statuses)]; // Remove duplicates
});

// Extract unique branches dynamically
const uniqueBranches = computed(() => {
    const branches = props.employees.map(employee => employee?.profile?.branch).filter(Boolean);
    return [...new Set(branches)]; // Remove duplicates
});

// Filter employees based on status and branch
const filteredEmployees = computed(() => {
    return props.employees.filter(employee => {
        const matchesStatus = selectedFilterStatus.value ? employee.status === selectedFilterStatus.value : true;
        const matchesBranch = selectedFilterBranch.value ? employee?.profile?.branch === selectedFilterBranch.value : true;
...
const confirmUpdateStatus = async () => {
    if (!selectedEmployee.value || !selectedStatus.value) return;

    try {
        await axios.put(`/api/employees/${selectedEmployee.value.id}/status`, {
