<template>
    <Head title="Manage Contributions" />
    ...
    <AuthenticatedLayout>
            ...
            <div class="flex justify-between items-center w-full max-w-7xl mx-auto">
                ...
                <div>
                    ...
                    <button @click="openUploadModal" class="bg-green-600 hover:bg-green-700 py-3 px-4 rounded-md text-white">
                        <i class="fa-solid fa-circle-dollar-to-slot mr-2"></i> Register New Employee
                    </button>
                </div>
            </div>
        </template>
                    ...
                    <div class="min-w-full inline-block align-middle">
                        <div class="overflow-hidden">
                            <table class="table-auto min-w-full rounded-xl">
                                ...
                                <tbody class="items-center divide-y divide-gray-300 ">
                                    <tr v-for="user in users" :key="user.id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                                        ...
                                        <td class="p-5  flex justify-center items-center">
                                            <button @click="openUpdateModal(user.deductions)" class="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
                                            ...
                                            <button @click="deleteDeduction(user.deductions.id)"  class="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-red-600 flex item-center">
        ...
        <div v-if="isUpdateModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg px-6 py-8 w-full max-w-md relative">
                ...
                <h3 class="text-lg font-semibold mb-4">Update Deduction</h3>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium">Pag-IBIG:</label>
                        <input v-model="updateForm.pag_ibig" type="number" class="border p-2 rounded-md w-full" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium">SSS:</label>
                        <input v-model="updateForm.sss" type="number" class="border p-2 rounded-md w-full" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium">PhilHealth:</label>
                        <input v-model="updateForm.philhealth" type="number" class="border p-2 rounded-md w-full" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Maxicare:</label>
                        <input v-model="updateForm.maxicare" type="number" class="border p-2 rounded-md w-full" />
                    </div>
                </div>
...
// Close add deduction modal
const closeUploadModal = () => {
    isUploadModalOpen.value = false;
    resetForm();
};

// Open update deduction modal
const openUpdateModal = (deduction) => {
    updateForm.value = { ...deduction };
    isUpdateModalOpen.value = true;
};

// Close update deduction modal
const closeUpdateModal = () => {
    isUpdateModalOpen.value = false;
    resetUpdateForm();
};

// Reset add deduction form
const resetForm = () => {
    form.value = {
        user_id: '',
        pag_ibig: 0,
...
const saveDeduction = async () => {
    try {
        ...
        toast.success('Deduction saved successfully!', { timeout: 3000 });
        ...
        toast.error('Failed to save deduction.', { timeout: 3000 });
    }
};
...
const updateDeduction = async () => {
    try {
        await axios.put(`/api/deductions/${updateForm.value.id}`, updateForm.value);
        toast.success('Deduction updated successfully!', { timeout: 3000 });
        ...
        console.error('Error:', error.response?.data || error.message);
        toast.error('Failed to update deduction.', { timeout: 3000 });
    }
};
...
const deleteDeduction = async (id) => {
    if (confirm('Are you sure you want to delete this deduction?')) {
        try {
            ...
            toast.success('Deduction deleted successfully!', { timeout: 3000 });
            ...
            console.error('Error:', error.response?.data || error.message);
            toast.error('Failed to delete deduction.', { timeout: 3000 });
        }
    }
};
</script>
