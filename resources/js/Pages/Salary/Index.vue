    ...
    <AuthenticatedLayout>
                                ...
                                <tbody class="divide-y divide-gray-300 ">
                                    <tr v-for="user in users"  :key="user.id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                                        ...
                                        <td class="flex p-5 items-center justify-center gap-2">
                                            <button @click="openUpdateModal(user.salary)" class="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
                                            ...
                                            <button @click="deleteSalary(user.salary.id)" class="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-red-600 flex item-center">
        ...
        <div v-if="isUpdateModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg px-6 py-8 w-full max-w-md relative">
                ...
                <h3 class="text-lg font-semibold mb-4">Update Salary</h3>

                <div class="grid grid-cols-2 gap-4 mb-2">
                    <div>
                        <label class="block text-sm font-medium">Rate Per Day:</label>
                        <input v-model="updateForm.rate_per_day" type="number" class="border p-2 rounded-md w-full" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Overtime Rate:</label>
                        <input v-model="updateForm.overtime_rate" type="number" class="border p-2 rounded-md w-full" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Holiday Rate:</label>
                        <input v-model="updateForm.holiday_rate" type="number" class="border p-2 rounded-md w-full" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Allowance:</label>
                        <input v-model="updateForm.allowance" type="number" class="border p-2 rounded-md w-full" />
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium">Monthly Rate:</label>
                    <input v-model="updateForm.monthly_rate" type="number" class="border p-2 rounded-md w-full" />
                </div>
...
const saveSalary = async () => {
    try {
        ...
        toast.success('Salary saved successfully!', { timeout: 3000 });
        ...
        toast.error(error.response?.data?.message || 'Failed to save salary.', { timeout: 3000 });
    }
};

// Open Update Modal
const openUpdateModal = (salary) => {
    updateForm.value = { ...salary };
...
const updateSalary = async () => {
    try {
        await axios.put(`/api/salaries/${updateForm.value.id}`, updateForm.value);
        toast.success('Salary updated successfully!', { timeout: 3000 });
        ...
        toast.error(error.response?.data?.message || 'Failed to update salary.', { timeout: 3000 });
    }
};
...
const deleteSalary = async (id) => {
    if (confirm('Are you sure you want to delete this salary?')) {
        try {
            ...
            toast.success('Salary deleted successfully!', { timeout: 3000 });
            ...
            toast.error(error.response?.data?.message || 'Failed to delete salary.', { timeout: 3000 });
        }
    }
};
</script>
