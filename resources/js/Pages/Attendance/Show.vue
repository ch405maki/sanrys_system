                    ...
                    <div class="min-w-full inline-block align-middle">
                        <div class="overflow-hidden">
                            <table id="Attendance" class="table-auto min-w-full rounded-xl">
                                <thead>
                                    <tr class="bg-gray-50">
                                        <th scope="col" class="p-2 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize min-w-[150px]"> Date </th>
                                        <th scope="col" class="p-2 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize min-w-[150px]"> Time In </th>
                                        <th scope="col" class="p-2 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize"> Time Out </th>
                                        <th scope="col" class="p-2 text-center whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize"> Action </th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-300 ">
                                    ...
                                    <tr v-for="attendance in filteredAttendances" :key="attendance.id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                                        ...
                                        <td class="flex p-2 items-center justify-center gap-0.5">
                                            <button title="Update" @click="openUpdateModal(attendance)" class="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
...
const closeUpdateModal = () => {
    ...
    updateForm.value = { id: null, time_in: '', time_out: '' };
};
...
const filteredAttendances = computed(() => {
    ...
    return props.user.attendances.filter(attendance => {
        return new Date(attendance.time_in).toISOString().slice(0, 7) === monthFilter.value;
    });
});
...
const exportToExcel = () => {
    ...
    if (!table) {
        toast.error('Error: Attendance table not found.');
        return;
    }
