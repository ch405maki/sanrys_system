    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center p-[5px]">
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Attendance / <span class="text-xs font-normal text-green-900">Manage Attendance</span></h2>
                </div>
                <div>
                    <!-- Search Section -->
                            ...
                            <table class="table-auto min-w-full rounded-xl">
                                ...
                                <tbody class="divide-y divide-gray-300 ">
                                    <tr v-for="user in users" :key="user.id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                                        ...
                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ user?.profile?.contact_number }} </td>
                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ user?.profile?.position }} </td>
                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ user?.profile?.branch }} </td>
                                        <td class="flex p-5 items-center justify-center gap-0.5">
                                            <a :href="route('attendance.show', user.id)" class="bg-green-100 text-sm hover:bg-green-200 py-3 px-4 rounded-md border border-green-500 text-gray-700"> 
                                                <i class="fa-solid fa-calendar-days"></i> Manage Time Entries
                                            </a>
                                        </td>
                                    </tr>
                                </tbody>
